...
Matriz matriz_densa, matriz_res;
int boolean = 1; //Verifica se ocorreu a multiplicação das matrizes

void* multi_esparsa_vetor(void *params) { //Função para multiplicar a matriz esparsa pelo vetor coluna
...
void* multi_esparsa_esparsa(void *params) { //Função para multiplicar a matriz esparsa por putra matriz esparsa
    Params p = *(Params*) params;
...
void* multi_esparsa_densa(void *params) { //Função para multiplicar a matriz esparsa por uma matriz densa
    Params p = *(Params*) params;
    int soma_linha = 0, tam, pos, i;
    tam = matriz_esparsa1[p.l].size();
    for (i=0; i<tam; i++) {
...
void menu(int op, Parametros p) { //Função para executar a opção escolhida
    int i, j, tam, qtdThreads;
    if (op == 0) {
        if (p.coluna_matriz_esparsa1 == p.tam_vetor) { //Verificar se a operação pode ser feita
            qtdThreads = p.linha_matriz_esparsa1; //Definindo o número de threads (uma para cada linha)
        ...
        if (p.coluna_matriz_esparsa1 == p.linha_matriz_esparsa2) { //Verificar se a operação pode ser feita
            qtdThreads = p.linha_matriz_esparsa1*p.coluna_matriz_esparsa2; //Definindo o número de threads, para cada linha que vai multiplicar uma coluna
            pthread_t thread[qtdThreads];
            Params params[qtdThreads];
        ...
        printf("+++ %d %d +++", p.coluna_matriz_esparsa1, p.linha_matriz);
        if (p.coluna_matriz_esparsa1 == p.linha_matriz) { //Verificar se a operação pode ser feita
            qtdThreads = p.linha_matriz_esparsa1*p.coluna_matriz; //Definindo o número de threads, para cada linha que vai multiplicar uma coluna
            pthread_t thread[qtdThreads];
            Params params[qtdThreads];
            int cnt = 0; //Contador para o número de threads
...
void inserir_matriz_esparsa(vector<pair<int,float>> *matriz_esparsa, int linha, int *coluna) { 
    ...
    int i, flag, pos;
    float elemento;
    for (i=0; i<linha; i++) { //Leitura da matriz esparsa
        flag = 1;
        while(flag) {
            scanf("%d %f", &pos, &elemento);
            if (elemento == -1 && pos == -1) flag = 0;
            else {
                matriz_esparsa[i].push_back(make_pair(pos, elemento));
                if (pos > (*coluna)) (*coluna) = pos;
            }
        }
    }
    (*coluna)++;
    printf("Matriz esparsa digitada:\n");
    print_matriz_esparsa(matriz_esparsa, linha);
    printf("\n");
    return;
}

void inserir_vetor(int tam) {
    printf("Digite os elementos do seu vetor coluna: ");
        vetor_coluna.clear();
        vetor_coluna.resize(tam);
        for (int i=0; i<tam; i++) {
...
void inserir_matriz_densa(int linha, int coluna) {
    int i, j;
    for (i=0; i<linha; i++) {
        for (j=0; j<coluna; j++) {
...
int main() {
    ...
    inserir_matriz_esparsa(matriz_esparsa1, parametros.linha_matriz_esparsa1, &parametros.coluna_matriz_esparsa1);

    if (op == 0) {
        printf("Quantidade de elementos do seu vetor coluna: ");
        scanf("%d", &parametros.tam_vetor);
        inserir_vetor(parametros.tam_vetor);
    }

    else if (op == 1) {
        printf("Quantidade de linhas da sua segunda matriz esparsa: ");
        scanf("%d", &parametros.linha_matriz_esparsa2);
        matriz_esparsa2 = (vector<pair<int,float>> *) malloc(parametros.linha_matriz_esparsa2 * sizeof(vector<pair<int,float>>));
        inserir_matriz_esparsa(matriz_esparsa2, parametros.linha_matriz_esparsa2, &parametros.coluna_matriz_esparsa2);
    }
        ...
        matriz_densa.l = parametros.linha_matriz; matriz_densa.c = parametros.coluna_matriz;
        ...
        inserir_matriz_densa(matriz_densa.l, matriz_densa.c);
    }
